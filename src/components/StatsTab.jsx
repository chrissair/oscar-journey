        ...
        {Object.keys(stats.genreStats).length === 0 ? (
          ...
          return (
            <table className="stats-table stats-table-sortable">
              <thead>
                <tr>
                  <th className="sortable" onClick={() => toggleSort('genre')}>{t('stats.genre')}{arrow('genre')}</th>
                  {raters.map(name => (
                    <th key={name} className="sortable" onClick={() => toggleSort('avg')}>{t('stats.average')}{arrow('avg')}</th>
                  ))}
                  <th className="sortable" onClick={() => toggleSort('films')}>{t('stats.films')}{arrow('films')}</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((g, i) => (
